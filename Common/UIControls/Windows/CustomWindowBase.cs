...
namespace UIControls
{
  public class CustomWindowBase : Window
  {
    ...
    public override void OnApplyTemplate()
    {
      ...
      this.MainMenu = this.GetRequiredTemplateChild<Menu>("MainMenu");
      this.ContextMenuButton = this.GetRequiredTemplateChild<Button>("ContextMenuButton");

      if (this.ContextMenuButton != null)
      {
        this.ContextMenuButton.Click += (s, e) => (s as Button).ContextMenu.IsOpen = true;
      }
      if (this.CloseButton != null)
      {
        this.CloseButton.Click += (s, e) => CloseWindow();
      }
      ...
      {
        this.RestoreButton.Click += (s, e) => this.ToggleWindowState();
      }
      if (this.MaximizeButton != null)
      {
        this.MaximizeButton.Click += (s, e) => this.ToggleWindowState();
      }
      ...
      foreach (var action in _defered)
        action.Execute();
    ...
    #region DP
    // property set 이후에 OnApplyTemplate()가 호출되는 경우 처리
    protected List<DeferredAction> _defered = new List<DeferredAction>();

    public static readonly DependencyProperty WindowContextMenuProperty
      = DependencyProperty.Register("WindowContextMenu", typeof(IEnumerable), typeof(CustomWindowBase), new PropertyMetadata(null, OnContextMenuChanged));

    public IEnumerable WindowContextMenu
    {
      get { return (IEnumerable)GetValue(WindowContextMenuProperty); }
      set { SetValue(WindowContextMenuProperty, value); }
    }
    private static void OnContextMenuChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
    {
      var win = dependencyObject as CustomWindowBase;
      var newChild = e.NewValue as IEnumerable;
      if (newChild != null && win.ContextMenuButton != null)
      {
        win.ContextMenuButton.Visibility = Visibility.Visible;
        win.ContextMenuButton.ContextMenu.ItemsSource = newChild;
      }
      else
      {
        win._defered.Add(new DeferredAction(() => { OnContextMenuChanged(dependencyObject, e); }));
      }
    }

    public static readonly DependencyProperty WindowMainMenuProperty
      = DependencyProperty.Register("WindowMainMenu", typeof(IEnumerable), typeof(CustomWindowBase), new PropertyMetadata(null, OnMainMenuChanged));

    public IEnumerable WindowMainMenu
    {
      get { return (IEnumerable)GetValue(WindowMainMenuProperty); }
      set { SetValue(WindowMainMenuProperty, value); }
    }
    private static void OnMainMenuChanged(DependencyObject dependencyObject, DependencyPropertyChangedEventArgs e)
    {
      var win = dependencyObject as CustomWindowBase;
      var newChild = e.NewValue as IEnumerable;
      if (newChild != null && win.MainMenu != null)
      {
        win.MainMenu.Visibility = Visibility.Visible;
        win.MainMenu.ItemsSource = newChild;
      }
      else
      {
        // 아직 loading이 끝나지 않았다
        win._defered.Add(new DeferredAction(() => { OnMainMenuChanged(dependencyObject, e); }));
      }
    }
    #endregion
